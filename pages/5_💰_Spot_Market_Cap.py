...
# Cache function to load Price Floor data from Google Sheets
@st.cache_data(ttl=3600)  # Cache for 1 hour
def load_price_floor_data():
    """Load price floor data from Google Sheets"""
    try:
        # Google Sheets CSV export URL for KaspaDiscordPricefloor sheet
        ...
        st.error(f"⚠️ Failed to load Google Sheets price floor data: {str(e)}")
...
def combine_price_datasets():
    """Combine price floor data with exchange data"""
    try:
        ...
        # Calculate days from genesis for floor data
        ...
        # Filter floor data to only include dates before exchange data starts
        floor_filtered = floor_df[floor_df['date'] < exchange_start_date].copy()
        
        # Prepare floor data to match exchange data format
        ...
        combined_df = pd.concat([floor_formatted, exchange_formatted], ignore_index=True)
        ...
        st.success(f"✅ Combined dataset: {len(floor_formatted)} price floor + {len(exchange_formatted)} exchange = {len(combined_df)} total points")
        
        return combined_df, genesis_date, floor_formatted
...
if 'combined_price_df' not in st.session_state or 'floor_data_df' not in st.session_state:
    st.session_state.combined_price_df, st.session_state.price_genesis_date, st.session_state.floor_data_df = combine_price_datasets()
...
<div class='subtitle-enhanced'>🔗 Combined Price Floor + Exchange Data | Complete Historical View</div>
...
if not filtered_df.empty:
    ...
    if show_data_sources == "Separate" and 'data_source' in filtered_df.columns:
        # Show price floor and exchange data separately
        pricefloor_data = filtered_df[filtered_df['data_source'] == 'pricefloor']
        ...
        # Add price floor data with different styling
        if not pricefloor_data.empty:
            floor_x = pricefloor_data['days_from_genesis'] if x_scale_type == "Log" else pricefloor_data['Date']
            
            fig.add_trace(go.Scatter(
                x=floor_x,
                y=pricefloor_data['Price'],
                mode='lines+markers',
                name='Discord Price Floor',
                line=dict(
                ...
                hovertemplate='<b>Discord Price Floor</b><br>Price: $%{y:.6f}<br>Date: %{text}<extra></extra>',
                text=[d.strftime('%B %d, %Y') for d in pricefloor_data['Date']],
                showlegend=True
            ))
...
<div class="metrics-container">
    ...
    <div class="metric-card">
        <div class="metric-label">Price Floor Points</div>
...
<div class="analysis-section">
    <div class="analysis-card">
        ...
        <ul class="insights-list">
            ...
            <li>Discord price floor data: <span class="data-quality-high">{floor_data_count}</span> historical data points</li>
            ...
            <li>Price floor data fills the critical first <strong>{floor_coverage_pct:.1f}%</strong> of price history</li>
...
if not combined_price_df.empty and 'data_source' in combined_price_df.columns:
    ...
    for source in source_timeline['data_source'].unique():
        ...
        color = '#ff8c00' if source == 'pricefloor' else '#5B6CFF'
        name = 'Discord Price Floor' if source == 'pricefloor' else 'Exchange Data'
...
<div class="analysis-section">
    <div class="analysis-card">
        ...
        <ul class="insights-list">
            <li><strong>Discord Price Floor (2021-11-17 to 2022-05-24):</strong> Community-established minimum price levels from Kaspa Discord</li>
            <li><strong>Price Floor Concept:</strong> Represents the lowest acceptable trading prices discussed in the community</li>
            <li><strong>Exchange Data (2022-05-25+):</strong> Official trading data from cryptocurrency exchanges</li>
            <li><strong>Transition Period:</strong> Overlapping data sources provide validation of price floor accuracy</li>
    ...
    <div class="analysis-card">
        ...
        <ul class="insights-list">
            ...
            <li>First ~200 days now captured through Discord price floor data</li>
            <li>Early price discovery period shows community-driven valuation methods</li>
            <li>Transition to exchange trading marked increased liquidity and price stability</li>
...
with col1:
    if st.button("🔄 Refresh Price Floor Data", key="refresh_sheets"):
...
<div style="margin-top: 3rem; padding: 1.5rem; background: rgba(26, 26, 46, 0.3); border-radius: 12px; border: 1px solid rgba(54, 54, 80, 0.4);">
    ...
    <p style="color: #9CA3AF; font-size: 0.85rem; margin: 0; text-align: center;">
        🔗 <strong>Google Sheets Integration:</strong> <a href="https://docs.google.com/spreadsheets/d/1hLOPPNUCPmBJ_IisrbCmkDJ3hjWiKG3tyx5ugtpB5cY/edit?gid=1073617241#gid=1073617241" 
        target="_blank" style="color: #5B6CFF;">KaspaDiscordPricefloor</a> | 
        💡 <strong>Enhanced Analysis:</strong> Combined dataset provides comprehensive price history from genesis to present
    </p>
</div>
""".format(floor_data_count=floor_data_count, exchange_data_count=exchange_data_count), unsafe_allow_html=True)
